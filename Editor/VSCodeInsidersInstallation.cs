...
{
	internal class VSCodeInsidersInstallation : LinuxIdeInstallation
	{
		...
		{
			// Currently supports VS Code Insiders, but infrastructure ready for other VS Code engine-based editors
			return File.Exists(path) && path.EndsWith("code-insiders", StringComparison.OrdinalIgnoreCase);
		}
		...
		public static bool TryDiscoverInstallation(string editorPath, out ILinuxIdeInstallation installation)
		{
			...
			installation = new VSCodeInsidersInstallation()
			{
				IsPrerelease = isPrerelease,
				Name = "Visual Studio Code Insiders" + (version != null ? $" [{version.ToString(3)}]" : string.Empty),
				Path = editorPath,
		...
		public static IEnumerable<ILinuxIdeInstallation> GetLinuxIdeInstallations()
		{
			...
			candidates.Add("/bin/code-insiders");
			candidates.Add("/usr/local/bin/code-insiders");
		...
		private static IEnumerable<string> GetXdgCandidates()
		{
			...
			var desktopFiles = new[] { "code-insiders.desktop" };
