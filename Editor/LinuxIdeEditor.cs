...
[assembly: InternalsVisibleTo("Unity.IdeLinux.EditorTests")]
[assembly: InternalsVisibleTo("Unity.IdeLinux.Standalone.EditorTests")]
[assembly: InternalsVisibleTo("DynamicProxyGenAssembly2")]

namespace Microsoft.Unity.IdeLinux.Editor
{
	[InitializeOnLoad]
	public class LinuxIdeEditor : IExternalCodeEditor
	{
		...
		private static readonly AsyncOperation<Dictionary<string, ILinuxIdeInstallation>> _discoverInstallations;

		static LinuxIdeEditor()
		{
			if (!UnityInstallation.IsMainUnityEditorProcess)
				return;

			Discovery.Initialize();
			CodeEditor.Register(new LinuxIdeEditor());

			_discoverInstallations = AsyncOperation<Dictionary<string, ILinuxIdeInstallation>>.Run(DiscoverInstallations);
		}
		...
		private static Dictionary<string, ILinuxIdeInstallation> DiscoverInstallations()
		{
			try
			{
				return Discovery
		...
		internal static bool IsEnabled => CodeEditor.CurrentEditor is LinuxIdeEditor && UnityInstallation.IsMainUnityEditorProcess;
		...
		{
			if (!TryGetLinuxIdeInstallationForPath(CodeEditor.CurrentEditorInstallation, true, out var installation))
				return;
		...
		internal virtual bool TryGetLinuxIdeInstallationForPath(string editorPath, bool lookupDiscoveredInstallations, out ILinuxIdeInstallation installation)
		{
		...
		{
			var result = TryGetLinuxIdeInstallationForPath(editorPath, lookupDiscoveredInstallations: false, out var vsi);
		...
		private static void RegenerateProjectFiles(ILinuxIdeInstallation installation)
		{
		...
		private static void SettingsButton(ProjectGenerationFlag preference, string guiMessage, string toolTip, ILinuxIdeInstallation installation)
		{
			var generator = installation.ProjectGenerator;
		...
		private static bool OpenFromInstallation(ILinuxIdeInstallation installation, string path, int line, int column)
		{
		...
		private static bool IsProjectGeneratedFor(string path, IGenerator generator, out ProjectGenerationFlag missingFlag)
		{
			...
			// The package manager requires relative path + unix style separators for queries
