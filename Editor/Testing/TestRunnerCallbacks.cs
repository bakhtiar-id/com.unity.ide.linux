...
namespace Microsoft.Unity.IdeLinux.Editor.Testing
{
	internal class TestRunnerCallbacks : ICallbacks
	{
		...
		{
			LinuxIdeIntegration.BroadcastMessage(Messaging.MessageType.RunFinished, Serialize(testResultAdaptor));
		}

		public void RunStarted(ITestAdaptor testAdaptor)
		{
			LinuxIdeIntegration.BroadcastMessage(Messaging.MessageType.RunStarted, Serialize(testAdaptor));
		}

		public void TestFinished(ITestResultAdaptor testResultAdaptor)
		{
			LinuxIdeIntegration.BroadcastMessage(Messaging.MessageType.TestFinished, Serialize(testResultAdaptor));
		}

		public void TestStarted(ITestAdaptor testAdaptor)
		{
			LinuxIdeIntegration.BroadcastMessage(Messaging.MessageType.TestStarted, Serialize(testAdaptor));
		}
		...
		internal void TestListRetrieved(TestMode testMode, ITestAdaptor testAdaptor)
		{
			...
			LinuxIdeIntegration.BroadcastMessage(Messaging.MessageType.TestListRetrieved, value);
		}
	}
}
