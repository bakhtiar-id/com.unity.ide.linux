...
using Microsoft.Unity.IdeLinux.Editor.Messaging;
using Microsoft.Unity.IdeLinux.Editor.Testing;
using UnityEditor;
using UnityEngine;
using MessageType = Microsoft.Unity.IdeLinux.Editor.Messaging.MessageType;

namespace Microsoft.Unity.IdeLinux.Editor
{
	[InitializeOnLoad]
	internal class LinuxIdeIntegration
	{
		class Client
		{
		...
		static LinuxIdeIntegration()
		{
			if (!LinuxIdeEditor.IsEnabled)
				return;
		...
		{
			// Note: Removed Windows-specific AppDomain.DomainUnload handler
			// Linux shutdown handling will be implemented as needed
		}
		...
		{
			var package = UnityEditor.PackageManager.PackageInfo.FindForAssembly(typeof(LinuxIdeIntegration).Assembly);
